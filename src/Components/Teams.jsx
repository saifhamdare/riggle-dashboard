  ...
  return (
    <div className="bg-white border-2 border-[#39CEF3] py-4 my-5">
      ...
      <div className="tab-container flex justify-evenly flex-wrap">
        <div className="">
        <div className="tab p-1 m-2 border-[#39CEF3] border-2 h-96">
          <h3 className="bg-[#39CEF3] text-white text-base text-center py-2 rounded-md">TEAM PERFORMANCE</h3>
          {teamData.achievement_leaderboard && teamData.achievement_leaderboard.map((item,idx)=>(

          
          <div key={item.id} className="team-member-tab flex items-center gap-5 border-b-2 border-slate-200 p-2  justify-between">
            <h4>{idx+1}</h4>
            <div className="avatar-container w-12   rounded-full">
              <picture>
            ...
            <div className="">
              <div className="employee-info sm:flex font-medium text-sm gap-3">
                <h2 className="text-sm">{item.first_name+" " +item.last_name} </h2>
                <h4 className=" text-sm ">( {item.designation_name} )</h4>
              </div>
              <div className="sales-info sm:flex text-[8px]  justify-evenly">
                <h2 className=" text-[10px] ">Target: {item.new_actiation_target}</h2>
        ...
        <div className="">
        <div className="tab p-1 m-2 border-[#39CEF3] border-2 h-96">
          <h3 className="bg-[#39CEF3] text-white text-base text-center py-2 rounded-md">NEW ACTIVATION</h3>
          {teamData.new_activation_leaderboard && teamData.achievement_leaderboard.map((item,idx)=>(

          
          <div key={item.id} className="team-member-tab flex gap-5 border-b-2 border-slate-200 p-2  justify-between">
            <h4>{idx+1}</h4>
            <div className="avatar-container w-12   rounded-full">
              <picture>
                <source srcSet={item.image} />
        ...
        <div className="">
        <div className="tab p-1 m-2 border-[#39CEF3] border-2 h-96">
          <h3 className="bg-[#39CEF3] text-white text-base text-center py-2 rounded-md">SKU PERFORMANCE (SET)</h3>
          {teamData.sku_achievement_leaderboard && teamData.achievement_leaderboard.map((item,idx)=>(
          ...
          <div key={item.id} className="team-member-tab flex gap-5 border-b-2 border-slate-200 p-2  justify-between">
            ...
            <div className="">
              ...
              <div className="sales-info sm:flex text-[8px]  justify-evenly">
                <h2 className=" text-[10px] ">Target: {item.new_actiation_target}</h2>
                <h4 className=" text-[10px] ">Achieved :{item.sku_achievement}</h4>
              </div>
            </div>
            <div className="">
                <div style={{boxShadow:'0px 4px 4px 0px #FFFFFF80 inset'}} className="bg-[#907EFF] border-2 w-7 h-7 border-[#907EFF] text-white text-[7px] px-1 py-2 rounded-full text-center">{item.achievement} %</div>
